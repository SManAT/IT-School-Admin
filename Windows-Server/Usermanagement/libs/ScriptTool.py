...
class ScriptTool():
    ...
    def modifyScript(self, lines, user):
        ...
        erg = []
        grpgOU = "CN=%s,%s" % (
            user.getGruppe(), self.config["ad"]["OU_GRUPPEN"])
        userDN = "CN=%s %s,OU=%s,%s" % (user.getNachname(), user.getVorname(
        ), user.getGruppe(), self.config["ad"]["OU_BENUTZER"])
        ...
        for line in lines:
            ...
            line = line.replace("%HOMEDIR%", self.pathEndingSlash(
                self.config["ad"]["HOME_PATH"]))
            line = line.replace("%PROFILEDIR%", self.pathEndingSlash(
                self.config["ad"]["PROFILE_PATH"]))
    ...
    def _execute(self, psTemplate, user):
        ...
        if psTemplate == "addUser.ps1":
            loginName = user.getUsername()
            if len(loginName) > 20:
                self.error.setErrorMessage(
                    "Error: SamAccountName %s is longer than 20 characters. -Skipping-\n" % loginName)
    ...
    def addUser(self, user):
        ...
        if self.groupExists(user) or self.debug is True:
            ...
            print("AD Group %s does not exists! User %s will be not created!" %
                  (user.getGruppe(), user.getFullname()))
